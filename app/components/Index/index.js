...
let currentUpdateEvent, currentVersion;
let timer1;
...
class Index extends Component {
    ...
    componentDidMount() {
        const _ol = document.getElementById('content');
        electron.ipcRenderer.on('message', (event, {message, data}) => {
            ...
            if ((currentUpdateEvent && currentUpdateEvent !== message) || !currentUpdateEvent) {
                ...
                _li.innerHTML = messageMap[message] +
                                (data !== undefined && ("<div>最新版本:" + data.version + "</div>") || "") +
                                (data !== undefined && ("<div>文件大小:" + (data.files[0].size / 1024 / 1024).toFixed(0) + "MB</div>") || "") +
    ...
    render() {
        return (
            <div className="p-index">
                <button onClick={this.autoUpdate}>检查更新</button>
                <ul id="content">
                    <p>生命周期过程展示:</p>
                </ul>
            </div>
